...
export default function AdminTransactions() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-[1200px]">
      ...
      <p className="text-muted-foreground mb-8">Click on the transaction to see all details</p>
      ...
      <div className="rounded-lg border">
        {/* Header */}
        <div className="grid grid-cols-[2fr,1fr,1fr] gap-4 p-4 bg-[#F8F9FC] border-b">
          <div className="font-medium text-sm text-muted-foreground">Template Name</div>
          <div className="font-medium text-sm text-muted-foreground">Created @</div>
          <div className="font-medium text-sm text-muted-foreground">Buyer User</div>
        </div>
        ...
        <div className="divide-y">
          {filteredTransactions.map((transaction, index) => (
            <div
              key={index}
              className="grid grid-cols-[2fr,1fr,1fr] gap-4 p-4 hover:bg-muted/50 cursor-pointer"
            >
              <div className="text-sm">{transaction.templateName}</div>
              <div className="text-sm">{transaction.createdAt}</div>
              <div className="text-sm">{transaction.buyerEmail}</div>
            </div>
          ))}
        </div>
      </div>
