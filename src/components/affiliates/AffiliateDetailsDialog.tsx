...
export function AffiliateDetailsDialog({ isOpen, onClose, affiliate, userUuid }: AffiliateDetailsDialogProps) {
  ...
  const { data: transactions = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Transaction[]> => {
      ...
      // Simplified query - just get transactions for this user
      ...
      return (data || []).map(transaction => ({
        ...
        product_name: 'Transaction' // We'll show a generic name for now
      }));
    },
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
          <DialogTitle className="text-2xl font-bold text-gray-900">{affiliate.name}</DialogTitle>
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow-sm h-full flex flex-col">
                ...
                <div className="bg-gray-50 border-b border-gray-200 px-4 py-3">
                  <div className="grid grid-cols-4 gap-4">
                    <div className="font-semibold text-gray-700">Transaction</div>
                    <div className="font-semibold text-gray-700 text-right">Amount</div>
